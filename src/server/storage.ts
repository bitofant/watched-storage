...
class WatchedStorage<T> {
	...
	public withSocketIO (io : SocketIO.Server) : WatchedStorage<T> {
	...
	public withAuthorizedSocketIO () : WatchedStorage<T> {
	...
	public restrictWriteAccess (listener : (socket: SocketIO.Socket, change: Events.Change) => boolean) : WatchedStorage<T> {
...
export { WatchedStorage };
export default WatchedStorage;
